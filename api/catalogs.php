...
try {
        ...
        if ($entity) {
            switch ($entity) {
                case 'categories':
                    ...
                    exit;
                case 'subcategories':
                    $stmt = $pdo->prepare('SELECT s.id,s.name,s.category_id,c.name as category_name, c.type as category_type FROM subcategories s JOIN categories c ON s.category_id=c.id WHERE s.user_id=:uid ORDER BY c.name,s.name');
                    $stmt->execute([':uid'=>$userId]);
                    echo json_encode(['success'=>true,'items'=>$stmt->fetchAll()]);
                    exit;
                case 'cost_centers':
                    ...
                    exit;
                case 'accounts':
                    $stmt = $pdo->prepare('SELECT id,name,initial_balance,is_default FROM accounts WHERE user_id=:uid ORDER BY name');
                    $stmt->execute([':uid'=>$userId]);
                    echo json_encode(['success'=>true,'items'=>$stmt->fetchAll()]);
                    exit;
                case 'payment_methods':
                    ...
                    exit;
                case 'fees':
                    $stmt = $pdo->prepare('SELECT f.id,f.name,f.percent,f.payment_method_id,pm.name as payment_method_name FROM fees f JOIN payment_methods pm ON pm.id=f.payment_method_id WHERE f.user_id=:uid ORDER BY pm.name,f.name');
                    $stmt->execute([':uid'=>$userId]);
                    echo json_encode(['success'=>true,'items'=>$stmt->fetchAll()]);
                    exit;
            }
    ...
    if ($method === 'POST') {
        ...
        switch ($entity) {
            ...
            case 'subcategories':
                $id = uniqid('sub_');
                if (empty($input['category_id']) || empty($input['name'])) throw new Exception('Categoria e nome são obrigatórios');
                $stmt = $pdo->prepare('INSERT INTO subcategories (id,user_id,category_id,name) VALUES (:id,:uid,:cat,:name)');
                $stmt->execute([':id'=>$id,':uid'=>$userId,':cat'=>$input['category_id'],':name'=>$input['name']]);
            ...
            case 'accounts':
                ...
                $check = $pdo->prepare('SELECT 1 FROM accounts WHERE user_id=:uid AND is_default=TRUE LIMIT 1');
                $check->execute([':uid' => $userId]);
                $hasDefault = $check->fetchColumn() ? true : false;
                if (!$hasDefault && !$isDefault) { $isDefault = true; }
                $stmt = $pdo->prepare('INSERT INTO accounts (id,user_id,name,initial_balance,is_default) VALUES (:id,:uid,:name,:initial,:def)');
                $stmt->execute([':id'=>$id,':uid'=>$userId,':name'=>$input['name'],':initial'=>$initial,':def'=>$isDefault]);
                if ($isDefault) {
                    // Garantir apenas uma conta padrão por usuário (PostgreSQL boolean)
                    $pdo->prepare('UPDATE accounts SET is_default=FALSE WHERE user_id=:uid AND id<>:id')->execute([':uid'=>$userId, ':id'=>$id]);
                }
            ...
            case 'fees':
                $id = uniqid('fee_');
                if (empty($input['payment_method_id']) || !isset($input['percent'])) throw new Exception('Forma de pagamento e percentual são obrigatórios');
    ...
    if ($method === 'PUT') {
        ...
        switch ($entity) {
            case 'categories':
                $stmt = $pdo->prepare('UPDATE categories SET name=:name WHERE id=:id AND user_id=:uid');
                $stmt->execute([':name'=>$input['name'] ?? '', ':id'=>$id, ':uid'=>$userId]);
                echo json_encode(['success'=>true]);
                exit;
            case 'subcategories':
                $stmt = $pdo->prepare('UPDATE subcategories SET name=:name WHERE id=:id AND user_id=:uid');
                $stmt->execute([':name'=>$input['name'] ?? '', ':id'=>$id, ':uid'=>$userId]);
                echo json_encode(['success'=>true]);
                exit;
            case 'cost_centers':
                $stmt = $pdo->prepare('UPDATE cost_centers SET name=:name WHERE id=:id AND user_id=:uid');
                $stmt->execute([':name'=>$input['name'] ?? '', ':id'=>$id, ':uid'=>$userId]);
                echo json_encode(['success'=>true]);
                exit;
            case 'accounts':
                ...
                $sql = 'UPDATE accounts SET '.implode(', ',$fields).' WHERE id=:id AND user_id=:uid';
                $stmt = $pdo->prepare($sql);
                $stmt->execute($params);
                if (array_key_exists('is_default',$input) && !!$input['is_default']) {
                    // Se esta conta virou padrão, desmarca as demais (PostgreSQL boolean)
                    $pdo->prepare('UPDATE accounts SET is_default=FALSE WHERE user_id=:uid AND id<>:id')->execute([':uid'=>$userId, ':id'=>$id]);
                }
                echo json_encode(['success'=>true]);
                exit;
            case 'payment_methods':
                $stmt = $pdo->prepare('UPDATE payment_methods SET name=:name WHERE id=:id AND user_id=:uid');
                $stmt->execute([':name'=>$input['name'] ?? '', ':id'=>$id, ':uid'=>$userId]);
                echo json_encode(['success'=>true]);
                exit;
        }
